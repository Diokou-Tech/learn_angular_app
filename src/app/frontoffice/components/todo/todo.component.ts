...
export class TodoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    localStorage.setItem('todos',JSON.stringify(this.langages));
  }
  idUser=1;
  user:any;
  users = [
  ...
  langages = [
      ...
      {name : 'HTML / CSS', id : 4, status: 'learned'},
      {name : 'C++', id : 5, status: 'not learned'},
  ];
  ...
  deleteLang(langId: number){
    ...
    confirm('Voulez vous supprimer cette langue') ?
    this.langages.splice(IndexElement,1): '';
  }
  addLang(){
    // alert('addLang click')
    if(this.langName == '' || this.langStatus == '' || this.langages.find((lang) => this.langName == lang.name)){
      alert('Remplir tous les champs ou ce nom existe déjà !');
      return ;
    }
